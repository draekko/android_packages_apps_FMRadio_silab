...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    /* Audio Output Status */
    private static boolean hasWiredHeadphones = false;
    private static boolean hasWiredHeadset = false;
    private static boolean hasBTA2DP = false;
    ...
    private static final AudioDeviceCallback mAudioDeviceCallback = new AudioDeviceCallback() {
        ...
        public void onAudioDevicesAdded(AudioDeviceInfo[] addedDevices) {
            ...
            Log.d(TAG, "Added audio devices: " + addedDevices.length);
            if (addedDevices.length > 0) {
                for (int ii = 0; ii < addedDevices.length; ii++) {
                    if (addedDevices[ii].getType() == AudioDeviceInfo.TYPE_WIRED_HEADPHONES) {
                        Log.d(TAG, "ADDED HEADPHONES");
                        hasWiredHeadphones = true;
                    }
                    if (addedDevices[ii].getType() == AudioDeviceInfo.TYPE_WIRED_HEADSET) {
                        Log.d(TAG, "ADDED HEADSET");
                        hasWiredHeadset = true;
                    }
                    if (addedDevices[ii].getType() == AudioDeviceInfo.TYPE_BLUETOOTH_A2DP) {
                        Log.d(TAG, "ADDED BLUETOOTH A2DP");
                        hasBTA2DP = true;
                    }
                }
            }
        ...
        public void onAudioDevicesRemoved(AudioDeviceInfo[] removedDevices) {
            ...
            Log.d(TAG, "Removed audio devices: " + removedDevices.length);
            if (removedDevices.length > 0) {
                for (int ii = 0; ii < removedDevices.length; ii++) {
                    if (removedDevices[ii].getType() == AudioDeviceInfo.TYPE_WIRED_HEADPHONES) {
                        Log.d(TAG, "REMOVED HEADPHONES");
                        hasWiredHeadphones = false;
                    }
                    if (removedDevices[ii].getType() == AudioDeviceInfo.TYPE_WIRED_HEADSET) {
                        Log.d(TAG, "REMOVED HEADSET");
                        hasWiredHeadset = false;
                    }
                    if (removedDevices[ii].getType() == AudioDeviceInfo.TYPE_BLUETOOTH_A2DP) {
                        Log.d(TAG, "REMOVED BLUETOOTH A2DP");
                        hasBTA2DP = false;
                    }
                }
            }
    ...
    private boolean powerUp(float frequency) {
        ...
        if (!FmNative.powerUp(frequency * 100)) {
            mPowerStatus = POWER_DOWN;
            return false;
        }
    ...
    private boolean tuneStation(float frequency) {
        if (mPowerStatus == POWER_UP) {
            setRds(false);
            boolean bRet = FmNative.tune(frequency * 100);
            if (bRet) {
                setRds(true);
                mCurrentStation = FmUtils.computeStation(frequency);
                FmStation.setCurrentStation(mContext, mCurrentStation);
    ...
    private float seekStation(float frequency, boolean isUp) {
        ...
        float fRet = FmNative.seek(frequency * 100, isUp);
        Log.d(TAG, "SEEK RET FREQ: "+fRet+"MHz");
    ...
    private void init() {
        ...
        if (mResolver == null) {
            ...
            Log.i(TAG, "Has Resolver");
        }
        mAudioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
        mActivityManager = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
        PowerManager powerManager = (PowerManager) getSystemService(Context.POWER_SERVICE);
        mWakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "FmRadio:"+TAG);
    ...
    private boolean isPatchMixerToEarphone(ArrayList<AudioPatch> patches) {
        ...
        if (getApplicationContext().getResources().getBoolean(R.bool.config_useSoftwareRenderingForAudio)) {
    ...
    private int updateDBInLocation(int[] stations) {
        ...
        try {
            ...
            cursor = mResolver.query(Station.CONTENT_URI,
    ...
    private boolean firstPlaying(float frequency) {
        ...
        float seekStation = FmNative.seek(frequency * 100, false);
        int station = FmUtils.computeStation(seekStation);
        if (FmUtils.isValidStation(station)) {
            isSeekTune = FmNative.tune(seekStation * 100);
            if (isSeekTune) {
